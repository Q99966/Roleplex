...
READ_ONLY = frozenset({'workspace_list', 'workspace_read', 'workspace_search', 'workspace_run_command',
                       'workspace_service_status', 'workspace_service_logs', 'memory_search', 'memory_read'})
...
def tool_evidence(name: str, status: str, private_output: dict | None) -> dict:
    ...
    """
    if isinstance(private_output, dict) and private_output.get('format') == 'not-dispatched-v1' and private_output.get('reason') in {'graph_budget','arguments_invalid','tool_unavailable'}:

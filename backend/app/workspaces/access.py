...
async def authorized_member(session: AsyncSession, *, conversation_id: int, role_id: int,
                            user_id: int | None, tool_name: str) -> tuple[Conversation, Role] | None:
    ...
    if (conversation is None or conversation.type != 'single' or conversation.deleted_at is not None
        ...
        or tool_name not in (role.builtin_tools_json or []) or user is None or not user.is_owner):
        return None
...
async def authorized_execution(session: AsyncSession, *, execution_id: str, conversation_id: int,
                               role_id: int, user_id: int | None, tool_name: str) -> tuple[Conversation, Role] | None:

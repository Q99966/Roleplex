...
def test_startup_archives_closed_dates_with_verified_manifest_and_audit(tmp_path: Path):
    """历史 runtime/unit/E2E 分类别归档，校验成功并审计后才删除源目录。"""
    from app.config.log_archive import maintain_logs
    from app.config.logging import configure_logging, shutdown_logging

    now = datetime(2026, 8, 26, 8, 0, tzinfo=timezone(timedelta(hours=8)))
    ...
    current_app = tmp_path / "runtime/2026-08-26/app.jsonl"
    events = [json.loads(line)["event"] for line in current_app.read_text().splitlines()]
...
def test_retention_and_size_delete_oldest_archive_with_planned_audit(tmp_path: Path):
    ...
    later = datetime(2026, 7, 31, 8, 0, tzinfo=timezone.utc)
    ...
    events = [json.loads(line) for line in (tmp_path / "runtime/2026-07-31/app.jsonl").read_text().splitlines()]
...
def test_running_e2e_and_current_date_are_never_archived(tmp_path: Path):
    """当前日期和 status=running 的 E2E 属于受保护事实。"""
    from app.config.log_archive import maintain_logs
    from app.config.logging import configure_logging, shutdown_logging

    now = datetime(2026, 8, 26, 8, 0, tzinfo=timezone.utc)
    current = tmp_path / "runtime/2026-08-26/app.jsonl"
...
def test_size_limit_prunes_oldest_recent_archive_first(tmp_path: Path):
    ...
    events = [json.loads(line) for line in (tmp_path / "runtime/2026-08-26/app.jsonl").read_text().splitlines()]

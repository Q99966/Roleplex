...
以下按 2026-09-23 的代码、测试入口与验证记录整理；本次世界基础的覆盖见[验收记录](docs/testing/world-orchestrator-foundation.md)，历史里程碑见[计划索引](docs/plan/README.md)。
...
| World 存档与类型 | 独立数据库和双密钥、界面/CLI 创建时选择已安装类型、配置与幂等初始化、专属页面、版本兼容与一致性备份；包装器切换 World。外部工作区不包含在备份中。 |
| 世界协调与桌宠 | Owner 就绪后建立固定世界管理者，可配置或导入模型/人设；独立持久协调对话、任务总览图、两群/类型活动委派、局部调整、反馈、根额度与精确停止。世界约定可主动查询、修订/停用并核对来源；保留投喂/抚摸。 |

原型或尚未形成产品功能的内容：MCP 生命周期与工具安全已有内部验证，尚无产品配置/调用闭环；Artifact 仅有原始内容读取与隔离原型，创建、更新、附件和前端预览尚未接入。Repository/Git/worktree、Checkpoint、全世界自动长期记忆抽取、会话导出/导入、重新生成和单条超长消息分块尚未实现。规划中的能力不表示已排定下一项任务。
...
点击桌宠打开世界协调面板，配置固定世界管理者的模型，或从已有角色导入独立配置后即可对话；选择“执行世界任务”授权委派，选择“补充当前任务”沿用原任务和剩余额度。流程视图展示世界目标与子任务，选中群可进入原工作流处理反馈。顶部停止控制准确任务树，收起面板不停止后台。详情集中放置上下文、调用用量与世界记忆，见[世界协调协议](docs/protocol/public/rest/world-orchestrator.md)。群内委派、系统派发和角色结果分别准确署名；同批任务以一条广播显示多个接收角色，各节点完整输入在执行详情中回读。历史输入保留原文并从重复共享材料中分离，见[协作通信协议](docs/protocol/public/rest/workflow-communication.md)。新类型扩展入口及受控样例见[世界类型协议](docs/protocol/public/rest/world-types.md)，Skills/MCP 不是接入前置。
...
复杂流程默认提供阶段总览，可展开并行区域和循环，折叠时仍显示等待、失败与反馈。节点细节支持按拓扑整理、撤销整理、定位节点和聚焦相关路径；已有手动位置默认保留。阶段名称、分支文案可保存，执行依赖、结果交接和反馈关联分层查看，展示分组不改变执行关系。

工作流主工具栏区分模板、本次运行调整、当前运行和历史只读；右侧只显示所选节点、连线或反馈的上下文。记录、图版本、草稿恢复和高级配置按需打开，窄屏使用详情抽屉。选中节点可直接提交反馈或请求协调者局部调整。

Owner 在右侧“会话成员”任命协调者，通过 `/plan@协调者 <要求>` 从目标创建/调整草稿，不自动启动；顶部运行方式选择“协调执行”后启动，允许协调者规划并执行；“让协调者调整运行”限定当前运行。整图写入和局部编辑是独立工具。普通发送与 @ 规则不变，手动流程无须任命。具体边界见[工作流协议](docs/protocol/public/rest/workflows.md)。

工作流节点可报告实现问题、契约冲突、能力缺口、未验证项和建议，在“节点反馈”中查看来源与处置。Owner 可自行核验或交给协调者；协调执行可显式启用自动处理，局部安排任务并在验证完成后复核。阻塞意见只暂停受影响的后继，原执行和文件事实保留，沿用本次运行预算。
...
- [世界类型与桌宠协调](docs/plan/world-orchestrator-foundation-v1.md)：公共实现与取舍；类型分支使用[worktree 接入说明](docs/plan/world-type-worktree-handoff-v1.md)。

...
以下按 2026-09-16 的代码、测试入口与既有验证记录整理；历史里程碑见[计划索引](docs/plan/README.md)。
...
| 会话工作流 | 右栏入口与覆盖聊天区的画布；保存串行角色任务和可选人工确认，冻结运行快照，查看消息/diff/用量，精确停止及核对后重试节点；群聊工具边界不变。 |
| 工作区与文件 | Owner 登记本机目录并绑定单聊或群聊，已有会话支持换绑和解绑，工作区和角色分别授权。支持列目录、搜索、按字节/行读取、批量读取、写入、自动创建父目录、局部和多片段编辑、批量修改、版本冲突与写入排队。群聊按 @ 顺序串行使用原生文件工具，命令、Shell 和后台服务工具仍限单聊。 |
...
原型或尚未形成产品功能的内容：MCP 生命周期与工具安全已有内部验证，尚无产品配置/调用闭环；Artifact 仅有原始内容读取与隔离原型，创建、更新、附件和前端预览尚未接入。Repository/Git/worktree、Orchestrator 并行编排、Checkpoint、长期记忆、会话导出/导入、重新生成和单条超长消息分块尚未实现。规划中的能力不表示已排定下一项任务。

会话内工作流从右侧“工作流”模块打开；画布覆盖主消息和输入区域，返回对话保留草稿与阅读位置，不停止运行。画布采用 React Flow，可编辑保存分支和环路；当前只运行用户明确编排的完整串行路径，尚不执行分支、并行或循环；操作与恢复规则见[工作流协议](docs/protocol/public/rest/workflows.md)。

...
图预算阻止工具派发时，会话保留“未执行”记录；已提交文件、结果未知和未派发分别展示，不增加回合总结或自动重放（小阶段 5，已通过人工验收）。

会话右栏提供概览与成员两个模块，通过圆形按钮展开模块菜单；桌面使用四分之一循环转盘、窄屏使用抽屉列表。进程保留独立入口，未开放扇区仅作不可选补位。

工作台采用浅蓝与薄荷绿配色；侧栏通过“会话／角色”切换完整列表，支持独立搜索和滚动位置保留。角色可按名称、标签和描述搜索，切换列表不改变当前会话与草稿。

角色设置的最大输出 Token 支持数字输入、快捷选项和厂商默认值；工具模块使用整行宽度，各类别可折叠并支持全选/取消全选及半选状态，不再展示高级 JSON 参数入口；已有其他参数仍保留，保存使用既有角色参数接口。上下文预算预留与实际输出上限分别说明。
...
不跨正文或写入归组，折叠摘要不隐藏失败。
批量写入/编辑已取消固定8文件与256 KiB参数限制（已人工验收）；完整逐文件结果保留，差异展示独立降级，前端支持分段展开。单文件大小、编辑片段与读取规则不变。
...
不生成每轮统计摘要，不向历史注入摘要。原工具卡和 Owner 私有详情按原权限与期限保存，中断后的按需事实交接已实现并通过人工验收：后续普通请求可取得同角色最近中断的执行事实，不依赖“继续”关键词，不自动重放或续跑。
...
默认仍为 8 的过渡值；时间/审批总预算、最终默认基准与原执行链自动续跑未纳入本轮实现，工具权限和停服规则不变。
...
Owner 可在设置中自定义新任务的决策次数（支持超过 256），或选择“不限次数”；默认仍为 8，已有任务保持发送时配置。原 `AGENT_DECISION_CEILING` 已退役、不再生效。不限只取消决策次数停止，权限、单次超时和用户停止保留；见[预算协议](docs/protocol/public/rest/agent-budget.md)。

工具参数错误恢复已修正并获准提交：字段/JSON 错误会保留未执行诊断并允许模型在原预算内修正；真正的协议/执行故障使用具体错误码。见[验证记录](docs/testing/tool-argument-recovery.md)。

右侧角色执行用量观测已通过人工验收，新增时间、Token 和费用限制暂缓。旧执行未采集的用量显示未知，不从日志回填。

原生文件操作在提交前后保存加密最小证据，进程中断后可核对当前版本；未知结果不冒充未执行。范围与限制见[中断事实契约](docs/protocol/internal/interruption-context.md)。

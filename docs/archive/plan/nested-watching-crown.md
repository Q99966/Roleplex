...
> 当前状态与替代入口见[现行摘要](../../plan/nested-watching-crown.md)；使用规则见[归档说明](../README.md)。

## 已完成：W1d 会话后台服务与 `/ps`（Linux 首版已人工验收）

用户希望现阶段补齐开发服务器保活、会话进程查询、日志和停止。范围、生命周期、默认值及验收集中见
[会话后台服务计划](conversation-services-v1.md)。已完成核心版、收尾及服务运行期间的 Shell 共存修订，用户已确认修复并授权提交；验证记录见该计划。
后续 W2a 尚未开始；
...
[工作区计划](agent-repository-workspaces-v1.md#w1c-实现与验证2026-09-10)。Shell 详情修正已一并通过人工验收并获准提交，W2a 尚未开始。

## 已完成追加任务：会话连接解耦与按需加载（A、B 均已人工验收）

阶段范围与验收统一见[会话连接解耦与按需加载计划](conversation-loading-v1.md)。A 连接与会话组件解耦已实现，
经用户人工验收后推进的 B 最近内容分页、缓存和滚动位置也已完成人工验收；单条超长消息分块等待内容协议成熟后另行确认。
该追加任务排在下一项工具能力实现之前，不改变下文工具与 Agent 主线的功能边界。

## 已完成：W1b 结构化基础命令（经人工验收）
...
- 日志归档策略于 2026-09-01 调整为当前月原始目录可见、进入新月份后归档上月；继续保留 30 天、总量目标
  1 GiB，以及归档/删除校验、审计和互斥边界。

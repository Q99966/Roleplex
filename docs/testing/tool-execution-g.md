...
|---|---|
| 受众 | 后端、前端与测试维护者 |
| 状态 | G1 自动、真实及人工验收通过，已获准提交；G2 候选试验完成；G3 预算/选型待确认 |
| 复核日期 | 2026-09-11 |
...
G3 仍需确认预算：保留当前候选时，write 的大文件 diff 会明确降级；E 可另验证利用已确认匹配位置生成局部 hunk，
...
G3 的库/预算选择仍需确认，D/E 未实施，不能把本次提交授权视为 diff 功能已交付或候选依赖已经接入产品。

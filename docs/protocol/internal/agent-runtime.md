...
图工具可用 `presentation` / `set_presentation` 提供展示阶段与分支文案；这些字段不进入结果求值或授予工具。当前有效图的纯展示修订保留正在规划/汇总的执行、原 phase、分工、预算与激活，不以画布编辑提前派发工作或重启汇总；节点仍引用实际执行设计版本。能力列表和每次提交的 Owner/群/任命检查保持不变。

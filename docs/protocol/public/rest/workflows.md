...
React Flow 负责选择、连线和位置；主工具栏明确模板、运行编辑、当前运行或历史只读对象，右栏按当前选择显示上下文。记录与高级配置按需打开，具体交互和浏览器草稿兼容规则见[前端设计](../../../design/frontend-navigation-theme.md)。Tab 在选中聚焦的编辑态节点后插入任务；Delete 仅删除选中连线或无连接节点。非编辑态和文本输入不触发删除。节点类型与颜色可修改；定义编辑不影响当前运行，运行图须通过明确修订入口更新。
...
世界协调者调用同一群规划/重规划服务，以持久 WorldTaskChild 附加真实父执行及根预算关联。每群保留自己的任命、能力分配、独立 run/chain 和人工确认规则；新世界委派不隐式复用其他任务的旧规划链。重规划引用准确 child/run/graph_revision 并沿用原子链；世界界面钻取原群画布处理反馈。协议见[世界协调](world-orchestrator.md)。

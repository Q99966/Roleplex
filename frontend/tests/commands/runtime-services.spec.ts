...
test('后台服务回复后仍可访问，/ps 不调用模型并能读取日志和停止', async ({ page }, testInfo) => {
  ...
  await page.evaluate(async (root) => {
    ...
    const role = await request('/api/roles', { name: '服务助手', system_prompt: '启动服务后使用登记工具管理。', model_config_id: config.id,
      model_name: 'fake-model', builtin_tools: ['workspace_start_service', 'workspace_service_status', 'workspace_service_logs', 'workspace_stop_service', 'workspace_run_shell'] })
  ...
  expect(await readFile(path.join(root, 'shell-proof.txt'), 'utf8').catch(() => null)).toBeNull()
  const approvalScreenshot = testInfo.outputPath('shell-with-service.png')
  await page.screenshot({ path: approvalScreenshot })
  await testInfo.attach('服务运行期间的 Shell 审批', { path: approvalScreenshot, contentType: 'image/png' })
  await page.getByRole('button', { name: '批准本次 Shell', exact: true }).click()
  await expect.poll(() => readFile(path.join(root, 'shell-proof.txt'), 'utf8').catch(() => '')).toBe('approved\n')
  await expect(page.getByRole('button', { name: '停止生成', exact: true })).toHaveCount(0)
  ...
  await expect(panel.getByRole('article').filter({ hasText: 'workspace_start_service' }).getByText('已停止', { exact: true })).toBeVisible()

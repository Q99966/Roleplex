...
test('真实写入按工具位置展示可折叠 diff，刷新恢复且 Guest 不获取私有正文', async ({ page }, testInfo) => {
  ...
  await expect(diff.locator('.diff-code-insert').first()).toHaveCSS('background-color', 'rgb(229, 243, 235)')
  await expect(diff.locator('.diff-code-delete').first()).toHaveCSS('background-color', 'rgb(250, 233, 235)')
  await expect(diff.locator('.diff-code-insert').first()).toHaveCSS('color', 'rgb(41, 69, 83)')

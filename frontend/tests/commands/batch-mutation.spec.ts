...
test('批量写入编辑独立文件 diff、拒绝不重写、刷新与 Guest 隔离', async ({ page }, testInfo) => {
  ...
  await expect(create.getByRole('region', { name: '文件差异：src/nested/batch-a.txt', exact: true })).toContainText('alpha old')
  await expect(create.getByText('本次创建了 2 个父目录。', { exact: true })).toHaveCount(2)
  const createdShot = testInfo.outputPath('created-parents.png')
  await create.screenshot({ path: createdShot })
  await testInfo.attach('自动创建父目录凭据', { path: createdShot, contentType: 'image/png' })
  ...
  await expect(batch.getByRole('region', { name: '文件差异：src/nested/batch-a.txt', exact: true })).toContainText('alpha new')
  await expect(batch.getByRole('region', { name: '文件差异：test/nested/batch-b.txt', exact: true })).toContainText('beta new')
  await batch.locator('summary').first().click()
  await expect(batch.getByRole('region', { name: '文件差异：src/nested/batch-a.txt', exact: true })).not.toBeVisible()
  await expect(batch.getByRole('region', { name: '文件差异：test/nested/batch-b.txt', exact: true })).toBeVisible()
  await batch.locator('summary').first().click()
  await expect(edit.getByText('原始输入与输出')).toHaveCount(0)
  expect(await readFile(path.join(root, 'src/nested/batch-a.txt'), 'utf8')).toBe('alpha new')
  expect(await readFile(path.join(root, 'test/nested/batch-b.txt'), 'utf8')).toBe('beta new')
  ...
  await expect(rejected.getByText('test/nested/batch-b.txt · 未执行', { exact: true })).toBeVisible()
  expect(await readFile(path.join(root, 'src/nested/batch-a.txt'), 'utf8')).toBe('alpha new')
  ...
  await expect(page.getByText('src/nested/batch-a.txt', { exact: false })).toHaveCount(0)
  await expect(page.getByText('本次创建了', { exact: false })).toHaveCount(0)

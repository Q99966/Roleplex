...
import { nodeColor } from './workflow-layout'
...
function WorkflowCanvas() {
  ...
  useEffect(() => { setDetail(null) }, [w.mode, w.run?.id, w.draft.definition.id])
  ...
  function commit(nodes: WorkflowNode[], edges = graph.edges.filter(([a, b]) => nodes.some(n => n.id === a) && nodes.some(n => n.id === b))) {
    w.update({ ...w.draft.definition, graph: { nodes, edges } })
  }
      ...
      {node && <aside aria-label="节点属性与结果" className="max-h-[55%] w-full shrink-0 overflow-y-auto border-t border-slate-800 bg-panel p-4 lg:max-h-none lg:w-72 lg:border-l lg:border-t-0">
        ...
        {w.mode === 'edit' ? <div className="space-y-3 text-xs">
          <label className="block">节点类型<select aria-label="节点类型" className={field} value={node.kind} onChange={e => {
            ...
            patch({ kind, role_id: kind === 'approval' ? null : role?.id ?? null,
              title: ['角色任务', '人工确认'].includes(node.title) ? (kind === 'role' ? '角色任务' : '人工确认') : node.title })
          }}><option value="role">角色任务</option><option value="approval">人工确认</option></select></label>
          ...
          {node.kind === 'role' && <>
            <label className="block">执行角色<select aria-label="执行角色" className={field} value={node.role_id ?? ''} onChange={e => patch({ role_id: Number(e.target.value) })}>
              <option value="" disabled>选择本会话角色</option>{roles.map(r => <option key={r.id} value={r.id}>{r.name}</option>)}</select></label>
          ...
          <fieldset><legend className="mb-1">交接哪些上游结果</legend>{graph.nodes.slice(0, graph.nodes.indexOf(node)).map(n => <label key={n.id} className="my-1 flex gap-2">
          ...
          <p className="text-slate-500">拖动只调整位置。可保存分支和环路，本轮仅运行完整串行路径。下方按钮仅用于串行重排，移到节点前的结果引用会移除。</p>
          ...
          {w.run?.attempts.filter(a => a.node_id === node.id).map(a => <div key={a.id} className="rounded-xl border border-slate-700 p-3">
            <p>尝试 {a.number} · {statusLabel(a.status)}{a.current ? ' · 当前' : ' · 历史结果'}</p>
            ...
            {a.current && a.status === 'waiting' && <button type="button" disabled={w.busy} className={`${button} mt-2`} onClick={() => void w.control({ action: 'confirm', attempt_id: a.id })}><Check size={12} className="mr-1 inline" />确认继续</button>}
          </div>)}
...
function AttemptDetails({ attempt, onClose }: { attempt: WorkflowAttempt; onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    const previous = document.activeElement as HTMLElement | null
    panel.current?.focus()
    Promise.all([workflows.message(w.conversation.id, w.run!.id, attempt.id), workflows.facts(w.conversation.id, w.run!.id, attempt.id)])
      .then(([msg, result]) => { if (live && epoch === getAuthEpoch()) { setMessage(msg); setFacts(result.text) } })
  ...
  return <div className="absolute inset-0 z-30 flex items-center justify-center bg-slate-950/60 p-3">
    <div ref={panel} tabIndex={-1} role="dialog" aria-modal="true" aria-label="节点尝试详情" className="max-h-full w-full max-w-2xl overflow-y-auto rounded-2xl border border-slate-700 bg-panel p-4 outline-none" onKeyDown={e => {
      ...
      {terminal && attempt.current && <div className="space-y-3 border-t border-slate-700 pt-3 text-xs">
